...
export default function UserDrawer(props: UserDrawerProps) {
  ...
  const rules = {
    userName: [
      ...
      { min: 2, max: 20, message: '用户名称长度必须介于 2 到 20 个字符', trigger: 'blur' },
    ],
    nickName: [
    ...
    password: [
      ...
      { min: 5, max: 20, message: '用户密码长度必须介于 5 到 20 个字符', trigger: 'blur' },
      {
    ...
    phonenumber: [
      { pattern: PHONE_PATTERN, message: '请输入正确的手机号码', trigger: 'blur' },
    ],
    roleIds: [
